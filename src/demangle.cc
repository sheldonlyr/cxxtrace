...
typedef struct
{
    ...
    // with the above two fields, so we use an int instead.  Amusingly it can
    // pack "signed bool" as expected, but relying on that to continue to be a
    // legal type seems ill-advised (as it's illegal in at least clang).
} ParseState;
...
public:
    ...
    // asked to demangle an mangled name from an untrusted source, so it should
    // be much larger than the largest expected symbol, but much smaller than
    // the amount of work we can do in, e.g., a second.
    //
...
static bool MaybeAppendDecimal(State* state, int val)
{
    ...
    if (state->parse_state.append) {
        // We can't have a one-before-the-beginning pointer, so instead start
        // with one-past-the-end and manipulate one character before the
        // pointer.
...
static bool ParseUnnamedTypeName(State* state)
{
    ...
    // }. Optionally parse the encoded value into 'which' and add 2 to get the
    // index.
    int which = -1;
    ...
    if (ParseOneCharToken(state, 'C')) {
        if (ParseCharClass(state, "1234")) {
            ...
            MaybeAppendWithLength(
                state, prev_name, state->parse_state.prev_name_length);
            return true;
        } else if (ParseOneCharToken(state, 'I') &&
                   ParseCharClass(state, "12") && ParseClassEnumType(state)) {
            return true;
        }
    }
    state->parse_state = copy;
...
static bool ParseType(State* state)
{
    ...
    //
    // CV-qualifiers overlap with some operator names, but an operator name is
    // not valid as a type.  To avoid an ambiguity that can lead to exponential
    // time complexity, refuse to backtrack the CV-qualifiers.
    //
    // _Z4aoeuIrMvvE
    //  => _Z 4aoeuI        rM  v     v   E
    ...
    // Similarly, these tag characters can overlap with other <name>s resulting
    // in two different parse prefixes that land on <template-args> in the same
...
static bool ParseBuiltinType(State* state)
{
    ...
    for (p = kBuiltinTypeList; p->abbrev != nullptr; ++p) {
        ...
        } else if (p->abbrev[2] == '\0' &&
                   ParseTwoCharToken(state, p->abbrev)) {
            MaybeAppend(state, p->real_name);
            return true;
        }
    }
...
static bool ParseTemplateArg(State* state)
{
    ...
    // parse all cases that can start this way jointly to share the common
    // prefix.
    //
    // We have:
    //
    ...
    //
    //   <expr-primary> ::= L <source-name> [<template-args>] <expr-cast-value>
    //   E
    //
    // Now inline both of these into <template-arg>:
    //
    //   <template-arg> ::= L <source-name> [<template-args>]
    //                  ::= L <source-name> [<template-args>] <expr-cast-value>
    //                  E
    //
...
static bool ParseUnresolvedName(State* state)
{
    ...
    if (ParseTwoCharToken(state, "sr") && ParseOneCharToken(state, 'N') &&
        ...
        OneOrMore(/* <unresolved-qualifier-level> ::= */ ParseSimpleId,
                  state) &&
...
static bool ParseExpression(State* state)
{
    ...
    // Parse the conversion expressions jointly to avoid re-parsing the <type>
    // in their common prefix.  Parsed as: <expression> ::= cv <type>
    // <conversion-args> <conversion-args> ::= _ <expression>* E
    //                   ::= <expression>
    //
    // Also don't try ParseOperatorName after seeing "cv", since
    // ParseOperatorName also needs to accept "cv <type>" in other contexts.
...
static bool ParseExprPrimary(State* state)
{
    ...
    // The "LZ" special case: if we see LZ, we commit to accept "LZ <encoding>
    // E" or fail, no backtracking.
...
static bool ParseExprCastValue(State* state)
{
    ...
    // have e.g. "7fffE", which will accept "7" as a number but then fail to
    // find the 'E'.
    ParseState copy = state->parse_state;
